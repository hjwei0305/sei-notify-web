...
@withRouter
@ResizeMe()
@connect(({ unit, loading }) => ({ unit, loading }))
class Unit extends Component {
  ...
  componentDidUpdate(_prevProps, prevState) {
    if (!isEqual(prevState.list, this.props.unit.list)) {
      this.setState({
        list: this.props.unit.list
      });
    }
  }
  ...
  render() {
    const { unit, loading } = this.props;
    const { delRowId, list } = this.state;
    const { showModal, rowData } = unit;
    const columns = [
      {
